...
* [listPointsOfSale](#listpointsofsale) - Listar puntos de venta
...
async function run() {
  const result = await lapyme.pointsOfSale.listPointsOfSale({});
...
import { pointsOfSaleListPointsOfSale } from "lapyme/funcs/points-of-sale-list-points-of-sale.js";
...
async function run() {
  const res = await pointsOfSaleListPointsOfSale(lapyme, {});
  if (res.ok) {
    const { value: result } = res;
    console.log(result);
  } else {
    console.log("pointsOfSaleListPointsOfSale failed:", res.error);
  }
}
