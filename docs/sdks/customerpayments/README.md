...
* [listCustomerPayments](#listcustomerpayments) - Listar cobranzas de clientes
* [createCustomerPayment](#createcustomerpayment) - Crear cobranza de cliente
* [getCustomerPayment](#getcustomerpayment) - Obtener cobranza de cliente
* [void](#void) - Anular cobranza de cliente
...
async function run() {
  const result = await lapyme.customerPayments.listCustomerPayments({});
...
import { customerPaymentsListCustomerPayments } from "lapyme/funcs/customer-payments-list-customer-payments.js";
...
async function run() {
  const res = await customerPaymentsListCustomerPayments(lapyme, {});
  if (res.ok) {
    const { value: result } = res;
    console.log(result);
  } else {
    console.log("customerPaymentsListCustomerPayments failed:", res.error);
  }
}
...
async function run() {
  const result = await lapyme.customerPayments.createCustomerPayment({
...
```typescript
import { LapymeCore } from "lapyme/core.js";
import { customerPaymentsCreateCustomerPayment } from "lapyme/funcs/customer-payments-create-customer-payment.js";
...
async function run() {
  const res = await customerPaymentsCreateCustomerPayment(lapyme, {
    idempotencyKey: "<value>",
    body: {
      customerId: "6c3bc874-5a0f-4681-9333-2cb3b1ae904e",
      pointOfSaleId: "4951289f-66e9-4bef-a8ed-fa5e1852a608",
  ...
  if (res.ok) {
    ...
    console.log("customerPaymentsCreateCustomerPayment failed:", res.error);
  }
}
...
async function run() {
  const result = await lapyme.customerPayments.getCustomerPayment({
...
```typescript
import { LapymeCore } from "lapyme/core.js";
import { customerPaymentsGetCustomerPayment } from "lapyme/funcs/customer-payments-get-customer-payment.js";
...
async function run() {
  const res = await customerPaymentsGetCustomerPayment(lapyme, {
    paymentId: "0feeb625-0d1b-4e20-9123-3c110b44163b",
  });
  if (res.ok) {
    const { value: result } = res;
    ...
    console.log("customerPaymentsGetCustomerPayment failed:", res.error);
  }
}
...
async function run() {
  const result = await lapyme.customerPayments.void({
...
```typescript
import { LapymeCore } from "lapyme/core.js";
import { customerPaymentsVoid } from "lapyme/funcs/customer-payments-void.js";
...
async function run() {
  const res = await customerPaymentsVoid(lapyme, {
    paymentId: "f88e9906-f672-4be8-8060-3195304503f9",
    idempotencyKey: "<value>",
  });
  if (res.ok) {
    ...
    console.log("customerPaymentsVoid failed:", res.error);
  }
}
