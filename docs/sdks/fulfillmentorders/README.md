...
* [start](#start) - Iniciar orden de preparación
* [cancelFulfillmentOrder](#cancelfulfillmentorder) - Cancelar orden de preparación
...
async function run() {
  const result = await lapyme.fulfillmentOrders.start({
...
import { fulfillmentOrdersStart } from "lapyme/funcs/fulfillment-orders-start.js";
...
async function run() {
  const res = await fulfillmentOrdersStart(lapyme, {
    fulfillmentOrderId: "b218ef54-a64b-4972-88b2-24e5c33d91be",
  });
  if (res.ok) {
    ...
    console.log("fulfillmentOrdersStart failed:", res.error);
  }
}
...
```typescript
import { LapymeCore } from "lapyme/core.js";
import { fulfillmentOrdersCancelFulfillmentOrder } from "lapyme/funcs/fulfillment-orders-cancel-fulfillment-order.js";
...
async function run() {
  ...
  if (res.ok) {
    ...
    console.log("fulfillmentOrdersCancelFulfillmentOrder failed:", res.error);
  }
}
