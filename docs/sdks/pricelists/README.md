...
* [createPriceList](#createpricelist) - Crear lista de precios
* [getPriceListById](#getpricelistbyid) - Obtener lista de precios por ID
* [updatePriceList](#updatepricelist) - Actualizar lista de precios
...
async function run() {
  const result = await lapyme.priceLists.createPriceList({
...
```typescript
import { LapymeCore } from "lapyme/core.js";
import { priceListsCreatePriceList } from "lapyme/funcs/price-lists-create-price-list.js";
...
async function run() {
  const res = await priceListsCreatePriceList(lapyme, {
    idempotencyKey: "<value>",
    body: {
      name: "<value>",
    },
  ...
  if (res.ok) {
    ...
    console.log("priceListsCreatePriceList failed:", res.error);
  }
}
...
async function run() {
  const result = await lapyme.priceLists.getPriceListById({
...
```typescript
import { LapymeCore } from "lapyme/core.js";
import { priceListsGetPriceListById } from "lapyme/funcs/price-lists-get-price-list-by-id.js";
...
async function run() {
  const res = await priceListsGetPriceListById(lapyme, {
    priceListId: "65f14589-07bb-4a1f-b411-cbf30c6616e1",
  });
  if (res.ok) {
    const { value: result } = res;
    ...
    console.log("priceListsGetPriceListById failed:", res.error);
  }
}
...
async function run() {
  const result = await lapyme.priceLists.updatePriceList({
...
```typescript
import { LapymeCore } from "lapyme/core.js";
import { priceListsUpdatePriceList } from "lapyme/funcs/price-lists-update-price-list.js";
...
async function run() {
  const res = await priceListsUpdatePriceList(lapyme, {
    priceListId: "7d551ebf-a140-4a50-96e2-83d830336188",
    body: {
      name: "<value>",
    },
  ...
  if (res.ok) {
    ...
    console.log("priceListsUpdatePriceList failed:", res.error);
  }
}
