...
* [listTags](#listtags) - Listar etiquetas
* [createTag](#createtag) - Crear etiqueta
* [updateTag](#updatetag) - Actualizar etiqueta
...
async function run() {
  const result = await lapyme.tags.listTags({
    scope: "product",
  });
...
import { tagsListTags } from "lapyme/funcs/tags-list-tags.js";
...
async function run() {
  const res = await tagsListTags(lapyme, {
    scope: "product",
  });
  if (res.ok) {
    ...
    console.log("tagsListTags failed:", res.error);
  }
}
...
async function run() {
  const result = await lapyme.tags.createTag({
...
```typescript
import { LapymeCore } from "lapyme/core.js";
import { tagsCreateTag } from "lapyme/funcs/tags-create-tag.js";
...
async function run() {
  const res = await tagsCreateTag(lapyme, {
    idempotencyKey: "<value>",
    body: {
      scope: "purchase",
      name: "<value>",
...
async function run() {
  const result = await lapyme.tags.updateTag({
...
```typescript
import { LapymeCore } from "lapyme/core.js";
import { tagsUpdateTag } from "lapyme/funcs/tags-update-tag.js";
...
async function run() {
  const res = await tagsUpdateTag(lapyme, {
    tagId: "d27ab3f2-d6df-4e6f-a838-daf199d4c1d8",
    body: {},
  });
  if (res.ok) {
    ...
    console.log("tagsUpdateTag failed:", res.error);
  }
}
