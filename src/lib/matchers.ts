...
import { LapymeDefaultError } from "../models/errors/lapyme-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | LapymeDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | LapymeDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new LapymeDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new LapymeDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
