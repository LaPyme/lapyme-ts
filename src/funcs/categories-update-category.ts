 ...
 */
export function categoriesUpdateCategory(
  client: LapymeCore,
  request: operations.UpdateApiCategoryRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.UpdateApiCategoryResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.UpdateApiCategoryRequest$outboundSchema, value),
  ...
  const pathParams = {
    category_id: encodeSimple("category_id", payload.category_id, {
      explode: false,
      charEncoding: "percent",
    }),
  };
  const path = pathToFunc("/api/v1/categories/{category_id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.UpdateApiCategoryResponse$inboundSchema, {
      key: "Result",
    }),
