 ...
 */
export function customerPaymentsGetCustomerPayment(
  client: LapymeCore,
  request: operations.GetApiCustomerPaymentRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.GetApiCustomerPaymentResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.GetApiCustomerPaymentRequest$outboundSchema, value),
  ...
  const pathParams = {
    payment_id: encodeSimple("payment_id", payload.payment_id, {
      explode: false,
      charEncoding: "percent",
    }),
  };
  const path = pathToFunc("/api/v1/customer-payments/{payment_id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetApiCustomerPaymentResponse$inboundSchema, {
      key: "Result",
    }),
