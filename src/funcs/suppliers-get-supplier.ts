 ...
 */
export function suppliersGetSupplier(
  client: LapymeCore,
  request: operations.GetApiSupplierRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.GetApiSupplierResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => z.parse(operations.GetApiSupplierRequest$outboundSchema, value),
  ...
  const pathParams = {
    supplier_id: encodeSimple("supplier_id", payload.supplier_id, {
      explode: false,
      charEncoding: "percent",
    }),
  };
  const path = pathToFunc("/api/v1/suppliers/{supplier_id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetApiSupplierResponse$inboundSchema, {
      key: "Result",
    }),
