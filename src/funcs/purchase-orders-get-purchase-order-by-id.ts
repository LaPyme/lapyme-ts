 ...
 */
export function purchaseOrdersGetPurchaseOrderById(
  client: LapymeCore,
  request: operations.GetApiPurchaseOrderByIdRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.GetApiPurchaseOrderByIdResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.GetApiPurchaseOrderByIdRequest$outboundSchema, value),
  ...
  const pathParams = {
    purchase_order_id: encodeSimple(
      "purchase_order_id",
      payload.purchase_order_id,
      { explode: false, charEncoding: "percent" },
    ),
  };
  const path = pathToFunc("/api/v1/purchase-orders/{purchase_order_id}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetApiPurchaseOrderByIdResponse$inboundSchema, {
      key: "Result",
    }),
