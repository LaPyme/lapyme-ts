 ...
 */
export function warehousesGetWarehouseById(
  client: LapymeCore,
  request: operations.GetApiWarehouseByIdRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.GetApiWarehouseByIdResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.GetApiWarehouseByIdRequest$outboundSchema, value),
  ...
  const pathParams = {
    warehouse_id: encodeSimple("warehouse_id", payload.warehouse_id, {
      explode: false,
      charEncoding: "percent",
    }),
  };
  const path = pathToFunc("/api/v1/warehouses/{warehouse_id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetApiWarehouseByIdResponse$inboundSchema, {
      key: "Result",
    }),
