 ...
 */
export function apiUpdateAPIWebhookEndpoint(
  client: LapymeCore,
  request: operations.UpdateApiWebhookEndpointRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.UpdateApiWebhookEndpointResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.UpdateApiWebhookEndpointRequest$outboundSchema, value),
  ...
  const pathParams = {
    webhook_endpoint_id: encodeSimple(
      "webhook_endpoint_id",
      payload.webhook_endpoint_id,
      { explode: false, charEncoding: "percent" },
    ),
  };
  const path = pathToFunc("/api/v1/webhook-endpoints/{webhook_endpoint_id}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.UpdateApiWebhookEndpointResponse$inboundSchema, {
      key: "Result",
    }),
